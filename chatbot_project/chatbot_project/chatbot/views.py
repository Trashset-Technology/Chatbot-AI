...
class ChatDataset(Dataset): # no use 
...
def load_intents(file_path):# no use
    try:
...
def train_chatbot(request):# no use
...
def home(request):
    # Always clear messages on GET request
    if request.method == 'GET':
        request.session['messages'] = []  # Clear messages on page load

    # Check if it's a new session
    if 'messages' not in request.session:
        request.session['messages'] = []  # Initialize message list in session

    if request.method == 'POST':
        user_message = request.POST.get('message')
        request.session['messages'].append({'text': user_message, 'type': 'user'})  # Add user message
        
        response = get_response(user_message)
        request.session['messages'].append({'text': response, 'type': 'bot'})  # Add bot response

        request.session.modified = True  # Mark session as modified

    messages = request.session['messages']  # Retrieve messages from session
